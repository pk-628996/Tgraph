...
async def uploadp(pk, message):
  ...
  await message.edit(text="Downloaded Successfully✅", message_id=message.message_id)
  try:
     tlink = upload_file(file)[0]
     await message.edit(text=f"https://telegra.ph{tlink} \n\n`https://telegra.ph{tlink}`\n\nTap the link to copy " , disable_web_page_preview=True, message_id=message.message_id)
     os.remove(file)
  except Exception as e:
     print(e)
